...
void assertTagMatches(T tagEnum, uint8_t tag) {
    ...
    assert(tag == tagEnumByte);
}

template<typename T>
void assertTagRange(T tagMin, T tagMax, uint8_t val) {
    assert(val >= static_cast<uint8_t>(tagMin) &&
...
Request::Request(const Ber::Packet p) {
    assertTagMatches<Ber::Tag>(Ber::Tag::Integer, p.children[0].tag);
    version = p.children[0];

    assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, p.children[1].tag);
    dn = std::string(p.children[1]);

    type = static_cast<Type>(static_cast<int>(p.children[2]));
    assert(type == Type::Simple || type == Type::Sasl);
    if (type == Type::Simple) {
...
Filter parseFilter(const Ber::Packet& p) {
    Filter ret;
    assertTagRange<Filter::Type>(Filter::Type::And, Filter::Type::Extensible, p.tag);
...
Request::Request(const Ber::Packet p) {
    // Basic sanity checks
    assertTagMatches<Ldap::MessageTag>(Ldap::MessageTag::SearchRequest, p.tag);
    assert(p.children.size() == 8);

    assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, p.children[0].tag);
    base = std::string(p.children[0]);

    assertTagMatches<Ber::Tag>(Ber::Tag::Enumerated, p.children[1].tag);
    assertTagRange<Scope>(Scope::Base, Scope::Sub, static_cast<uint64_t>(p.children[1]));
    scope = static_cast<Scope>(static_cast<uint64_t>(p.children[1]));

    assertTagMatches<Ber::Tag>(Ber::Tag::Enumerated, p.children[2].tag);
    assertTagRange<DerefAliases>(DerefAliases::Never, DerefAliases::Always,
            static_cast<uint64_t>(p.children[2]));
    derefAliases = static_cast<DerefAliases>(static_cast<uint64_t>(p.children[2]));

    assertTagMatches<Ber::Tag>(Ber::Tag::Integer, p.children[3].tag);
    sizeLimit = p.children[3];

    assertTagMatches<Ber::Tag>(Ber::Tag::Integer, p.children[4].tag);
    timeLimit = p.children[4];

    assertTagMatches<Ber::Tag>(Ber::Tag::Boolean, p.children[5].tag);
    typesOnly = p.children[5];

    filter = parseFilter(p.children[6]);

    assertTagMatches<Ber::Tag>(Ber::Tag::Sequence, p.children[7].tag);
    for (auto & a: p.children[7].children) {
        assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, a.tag);
        std::string attr(a);
...
Entry parseRequest(Ber::Packet p) {
    assertTagMatches<Ldap::MessageTag>(Ldap::MessageTag::AddRequest, p.tag);
    assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, p.children[0].tag);
    Entry ret(p.children[0]);

    Ber::Packet attrs = p.children[1];
    assertTagMatches<Ber::Tag>(Ber::Tag::Sequence, attrs.tag);
    for (auto attrSeq: attrs.children) {
        assertTagMatches<Ber::Tag>(Ber::Tag::Sequence, attrSeq.tag);
        assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, attrSeq.children[0].tag);
        assertTagMatches<Ber::Tag>(Ber::Tag::Set, attrSeq.children[1].tag);
        for (auto attrVal: attrSeq.children[1].children) {
            assertTagMatches<Ber::Tag>(Ber::Tag::OctetString, attrVal.tag);
...
std::string parseRequest(Ber::Packet p) {
    assertTagMatches<Ldap::MessageTag>(Ldap::MessageTag::DelRequest, p.tag);
